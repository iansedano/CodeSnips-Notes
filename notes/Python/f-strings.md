...
def name_format(full_name):
    ...
    first_name = full_name[0:space].capitalize()
    last_name = full_name[space + 1:].upper()
    return last_name + ', ' + first_name

my_name = 'Ian Currie'

print(name_format(my_name))

for entry in office:
    formatted_name = name_format(entry['full_name'])
    item = entry['item']
    print(f"{formatted_name:<30}    {item:<40}")
